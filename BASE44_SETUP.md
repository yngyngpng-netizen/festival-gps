...
- `base44/entities/CrewMember.jsonc` stores group members and schedules.
...
The `CrewMember` entity uses row-level security so signed-in users can read members whose group code matches their own `festivalGroupCode`, and can update only the member record they created.
